...
theoffsetmagic: int = getthemagicoffset()
...
class therobbloxhacker:
    def __init__(self):
        ...
        self.processhandle: Optional[int] = None
        self.modulebase: int = 0
        self.modulesize: int = 0
    ...
    def isvalidptr(self, ptr: int) -> bool:
        return minvalidptr <= ptr <= 0x7FFFFFFFFFFF
    ...
    def readnodeentry(self, nodeptr: int) -> Optional[bytes]:
        if not self.isvalidptr(nodeptr) or self.processhandle is None:
            return None
    ...
    def attachingandfuckingroblox(self):
        ...
        handle, base, size = self.memory.attachprocess(
            "RobloxPlayerBeta.exe",
            "RobloxPlayerBeta.exe",
        )
        if handle is None or base is None or size is None:
            raise RuntimeError("Failed to attach to Roblox process")
    ...
    def getsingleton(self) -> int:
        ...
        addr: int = self.modulebase + theoffsetmagic
    ...
    def _gethashmapinfo(self, singleton: int) -> Optional[Tuple[int, int, int]]:
        """Returns (mapend, maplist, mapmask) or None on failure."""
        if self.processhandle is None:
            return None
        ...
        if mapmask == 0 or maplist == 0 or not self.isvalidptr(maplist):
            return None
        return mapend, maplist, mapmask

    def _checkcachednode(self, namebytes: bytes, meta: Dict[str, int]) -> int:
        """Check if cached node matches the flag name. Returns getset ptr or 0."""
        cachednode: int = meta.get("nodeptr", 0)
        if not cachednode or not self.isvalidptr(cachednode):
            return 0
        ...
        if entrylen != len(namebytes) or entrynamebytes != namebytes:
            return 0
            
        getset = int.from_bytes(entrydata[offentrygetset:offentrygetset+8], 'little')
        if not self.isvalidptr(getset):
            return 0
        return getset

    def _checknodematch(self, entrydata: bytes, namebytes: bytes) -> int:
        """Check if entrydata matches namebytes. Returns getset ptr or 0."""
        entrynamebytes, entrylen = self.readentrynamebytes(entrydata)
        if entrylen != len(namebytes) or entrynamebytes != namebytes:
            return 0
        getset = int.from_bytes(entrydata[offentrygetset:offentrygetset+8], 'little')
    ...
    def _searchhashchain(self, nodecurrent: int, name: str, namebytes: bytes, 
                         bucketindex: int) -> int:
        """Search through hash chain for matching flag. Returns getset ptr or 0."""
        visited: set[int] = set()

        for _ in range(maxchainsafety):
            if nodecurrent in visited or len(visited) >= maxchainsteps:
                break
            ...
            getset = self._checknodematch(entrydata, namebytes)
            if getset:
                self.lookupmetacache[name] = {"bucketindex": bucketindex, "nodeptr": nodecurrent}
                self.cachevalueptr(name, getset)
                return getset

            forward = int.from_bytes(entrydata[offentryforward:offentryforward+8], 'little')
            if not forward or nodecurrent == forward or not self.isvalidptr(forward):
                return 0
            nodecurrent = forward
    ...
    def whereistheflagat(self, name: str) -> int:
        ...
        hashinfo = self._gethashmapinfo(singleton)
        if not hashinfo:
            return 0
        
        mapend, maplist, mapmask = hashinfo
        ...
        if not self.isvalidptr(nodecurrent) or nodecurrent == mapend:
            return 0

        # Check cached node first
        getset = self._checkcachednode(namebytes, meta)
        if getset:
            return getset

        # Search through hash chain
        return self._searchhashchain(nodecurrent, name, namebytes, bucketindex)

    def changesthetext(self, name: str, value: str) -> bool:
        addr = self.whereistheflagat(name)
        if not addr or self.processhandle is None:
            return False
    ...
    def _handlestringflag(self, key: str, cleanname: str, val: str | int | bool) -> Tuple[bool, str]:
        if self.changesthetext(cleanname, str(val)):
            return True, f"[ + ] {key} = \"{val}\""
        return False, f"[ - ] Failed: {key}"

    def _handleintflag(self, key: str, cleanname: str, val: str | int | bool) -> Tuple[bool, str]:
        try:
            targetvalue = int(val)
        except ValueError:
    ...
    def _handleboolflag(self, key: str, cleanname: str, val: str | int | bool) -> Tuple[bool, str]:
    ...
    def doitnow(self, key: str, val: str | int | bool) -> Tuple[bool, str]:
        cleanname, flagtype = self.themainfunctionlol(key)

        try:
            if flagtype == "string":
                return self._handlestringflag(key, cleanname, val)
            elif flagtype == "int":
                return self._handleintflag(key, cleanname, val)
            elif flagtype == "bool":
                return self._handleboolflag(key, cleanname, val)
            else:
...
def main():
    ...
    print(thebanner)
    print("[ + ] Icy FFlag Injector - https://discord.gg/dpMcfUKu \n")
